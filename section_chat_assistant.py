"""
Section-Specific AI Chat Assistant - Gemini 3 Hackathon Edition
Uses Google Gemini API for intelligent drug discovery responses
"""
...
try:
    ...
    model_name = os.getenv('GEMINI_MODEL', 'gemini-3-flash-preview')  # Read from .env
    if api_key:
        ...
        print("GEMINI_API_KEY not found in environment - using fallback")
        GEMINI_AVAILABLE = False
...
SECTION_CONTEXTS = {
    "basic_analysis": SectionContext("basic_analysis", "Basic Molecular Analysis",
        "You are a pharmaceutical chemistry expert. Help with molecular properties, drug-likeness, and basic chemical analysis. Be concise and scientific.",
        "🧬 Gemini AI Ready!", 
    ...
    "multiagent_analysis": SectionContext("multiagent_analysis", "Multi-Agent Drug Discovery",
        "You are an AI drug discovery expert specializing in multi-agent systems. Explain agent coordination, docking analysis, and computational drug design. Be technical but clear.",
        "🤖 Gemini AI Ready!", 
        ["Agent coordination?", "Docking analysis?", "Why multi-agent?"]),
    "rl_generation": SectionContext("rl_generation", "RL Molecule Generation",
        "You are a reinforcement learning expert for molecular design. Explain RL algorithms, reward functions, and molecule optimization. Be technical.",
        "🎯 Gemini AI Ready!", 
        ["How does RL work?", "Reward functions?", "Why use RL?"]),
    "multitarget_rl": SectionContext("multitarget_rl", "Multi-Target RL",
        "You are an expert in multi-objective optimization for drug discovery. Explain Pareto optimization, multi-target binding, and trade-offs.",
        "🎯 Gemini AI Ready!", 
    ...
    "chemical_space": SectionContext("chemical_space", "Chemical Space Analytics",
        "You are a cheminformatics expert. Explain chemical space exploration, molecular fingerprints, clustering, and similarity metrics.",
        "📊 Gemini AI Ready!", 
    ...
    "stability_analysis": SectionContext("stability_analysis", "Stability and ADMET",
        "You are an ADMET prediction expert. Explain absorption, distribution, metabolism, excretion, toxicity, and drug stability analysis.",
        "💊 Gemini AI Ready!", 
    ...
    "drug_compatibility": SectionContext("drug_compatibility", "Drug Compatibility",
        "You are a drug interaction expert. Explain drug-drug interactions, CYP450 metabolism, protein binding, and polypharmacy risks.",
        "⚠️ Gemini AI Ready!", 
...
def get_ai_response(user_message: str, section: SectionContext, file_content: str = None) -> str:
    """Generate response using Gemini API with fallback to knowledge base"""
    
    # Try Gemini first
    if GEMINI_AVAILABLE and gemini_model:
        try:
            # Build context-aware prompt
            context = f"""You are an expert in {section.section_name} for drug discovery.
            
System context: {section.system_prompt}

User question: {user_message}

{"Additional file context: " + file_content[:1000] if file_content else ""}

Provide a helpful, concise, scientific response in 2-4 sentences. Focus on practical insights."""
    ...
    return f"📚 {fallbacks.get(section.section_id, 'Please ask about specific topics like LogP, ADMET, docking, or molecular optimization.')}"


def render_chat_expander(section_id: str, molecule_context: Dict = None, key_suffix: str = ""):
    """Render chat UI with Gemini-powered responses"""
    ...
    with st.expander(f"💬 Ask Gemini AI about {section.section_name}", expanded=True):
        # Status
        if GEMINI_AVAILABLE:
            st.success("✅ Gemini 3 Connected - Real AI Responses")
        else:
            st.warning("⚠️ Gemini unavailable - Using knowledge base")
